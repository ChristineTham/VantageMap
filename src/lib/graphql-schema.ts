...
const FactSheetTypeEnum = new GraphQLEnumType({
  name: "FactSheetType",
  values: {
...
const LifecycleEnum = new GraphQLEnumType({
  name: "Lifecycle",
  values: {
    Plan: { value: "Plan" },
...
const HealthEnum = new GraphQLEnumType({
  name: "Health",
  values: {
    Good: { value: "Good" },
...
async function paginatedQuery(table: any, args: { page: number; pageSize: number; search?: string }, entityType: string) {
  const page = Math.max(1, args.page);
  const pageSize = Math.min(200, Math.max(1, args.pageSize));
  const offset = (page - 1) * pageSize;

  const conditions = args.search
    ? ilike(table.name, `%${args.search}%`)
    : undefined;
...
const QueryType = new GraphQLObjectType({
  ...
  fields: {
    ...
    capability: {
      ...
      args: { id: { type: new GraphQLNonNull(GraphQLString) } },
      resolve: async (_, { id }) => {
        const [row] = await db.select().from(businessCapabilities).where(eq(businessCapabilities.id, id)).limit(1);
    ...
    relationships: {
      ...
      resolve: async (_, args) => {
        const conditions = [];
        if (args.sourceType) conditions.push(eq(relationships.sourceType, args.sourceType as never));
        if (args.sourceId) conditions.push(eq(relationships.sourceId, args.sourceId));
        if (args.targetType) conditions.push(eq(relationships.targetType, args.targetType as never));
        if (args.targetId) conditions.push(eq(relationships.targetId, args.targetId));
        if (args.relationshipType) conditions.push(eq(relationships.relationshipType, args.relationshipType as never));
